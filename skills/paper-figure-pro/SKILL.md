...
**路由规则 (刚性):**
- **禁止多线聚集**: 单个 junction 最多出 2 根线。3 线以上必须拆分。
- **局部分流**: 并列模块(如日/周)在它们之间放 junction，就近左右分出。不从远处拉线。
- **理解用意再画**: 每个 junction 的位置服务于它所连接的模块——放在模块之间而非头顶。
- 输入干线: X → j1(主干) → j2(局部分流) / stfe_in(另一支)
- 所有分流/汇合用 junction 小黑点，不直接从形状拉多根线
